...
def scan_files_in_flat_folder(folder_path: str) -> List[Dict]:
    ...
    for file in files:
        try:
            ...
            import traceback
            log_error(LogTags.SCANNER, f"Error processing file '{file}' in folder {folder_path}", file=file, folder=folder_path, error=str(exc), traceback=traceback.format_exc())
            continue
    ...
    for base_name, files in groups.items():
        try:
            ...
            import traceback
            log_error(LogTags.SCANNER, f"Error parsing file group '{base_name}' in folder {folder_path}", base_name=base_name, folder=folder_path, error=str(exc), traceback=traceback.format_exc())
            continue

    return assets_dict


def scan_files_in_nested_folders(folder_path: str) -> Optional[List[Dict]]:
    ...
    try:
        ...
        for dir_entry in entries:
            ...
            try:
                ...
                files = [
                    f.name for f in os.scandir(dir_entry.path) 
                    if f.is_file() 
                ...
                import traceback
                log_error(LogTags.SCANNER, f"Failed to scan nested folder: {dir_entry.path}", path=dir_entry.path, error=str(exc), traceback=traceback.format_exc())
                continue
            if not files:
            ...
            try:
                ...
                import traceback
                log_error(LogTags.SCANNER, f"Failed to parse folder group: {dir_entry.path}", path=dir_entry.path, error=str(exc), traceback=traceback.format_exc())
                continue
    except Exception as exc:
        import traceback
        log_error(LogTags.SCANNER, f"Error scanning folder {folder_path}", folder=folder_path, error=str(exc), traceback=traceback.format_exc())
        return None
    return assets_dict
...
def process_files(folder_path: str) -> Optional[List[Dict]]:
...
def _is_asset_folders(folder_path: str) -> bool:
    ...
    try:
        ...
        log_error(LogTags.SCANNER, f"Error checking asset folders in {folder_path}", folder=folder_path, error=str(exc))
        return False
