...
def _fallback_keys(db: Session) -> Optional[tuple[set, set]]:
    """((group, tmdb_id) still on a fallback style, {groups the rename processed}).
    ...
    if not isinstance(fallbacks, dict):
        return None
    keys: set = set()
    for items in fallbacks.values():
        for it in items or []:
            tid = it.get("tmdb_id")
            if tid:
                keys.add((_norm_group(it.get("type", "")), int(tid)))
    scanned: set = set()
    for bucket, group in (("movies", "movie"), ("series", "series"), ("collections", "collection")):
        if int(data.get(bucket) or 0) > 0:
            scanned.add(group)
    return keys, scanned
...
def reconcile_community_lists(db: Session, sources: Iterable[str]) -> int:
    ...
    try:
        ...
        # Per source: (still-outstanding keys, scanned groups). A None result
        # means no fresh data — drop that source so we can't wrongly remove.
        outstanding: dict[str, tuple[set, set]] = {}
        if "unmatched" in wanted:
            res = _unmatched_keys(db)
            if res is None:
                wanted.discard("unmatched")
            else:
                outstanding["unmatched"] = res
        if "style_fallback" in wanted:
            res = _fallback_keys(db)
            if res is None:
                wanted.discard("style_fallback")
            else:
                outstanding["style_fallback"] = res
        if not wanted:
            return 0
        ...
        for it in _fetch_wanted_items(discord_id, wanted):
            ...
            keys, scanned = outstanding[src]
            group = _norm_group(media_type)
            # Resolved only if this type was actually scanned (partial-scan guard)
            # AND it's no longer in the outstanding set.
            if group in scanned and (group, int(tid)) not in keys:
